...
impl<R: CubeRuntime> IntTensorOps<Self> for CubeBackend<R> {
    ...
    fn int_matmul(lhs: IntTensor<Self>, rhs: IntTensor<Self>) -> IntTensor<Self> {
        let int8 = |dt: DType| matches!(dt, DType::I8 | DType::U8);
        if int8(lhs.dtype) && int8(rhs.dtype) {
            // ONNX MatMulInteger / e5 int8: keep u8/i8 until the kernel, accumulate i32.
            // Autotune would compile many candidates per e5 shape; CpuGemm is the
            // routine that actually walks 8-bit tiles.
            matmul(lhs, rhs, None, MatmulStrategy::CpuGemm, DType::I32).unwrap()
        } else {
            let dtype = lhs.dtype;
