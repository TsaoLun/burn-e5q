...
impl BurnGraph {
    ...
    fn build_scope(&mut self) {
        ...
        // ScalarTensor outputs were converted to ScalarNative at the boundary
        // (see convert_graph_boundary_scalars), so also match names present in
        // boundary_output_conversions — their `into_scalar` conversion still
        // consumes the on-device value and must count as a use.
