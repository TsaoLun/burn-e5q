...
/// one kernel (same dtype is the `EL = ER = EA = E` case, where the casts fold away).
///
/// Integer `u8`/`i8` → `i32` is the same path: inputs stay 8-bit through the tile walk and
/// widen only at the product. With a host-native LLVM TM this is what LoopVectorize can
/// turn into AVX512-VNNI; without it the leaf is still a correct tiled i32 accumulate.
/// What runs at the last level is the space's own statement, so nothing about it is spelled here.
#[cube(launch)]
